...
Ingress is examinable and a perennial confusion point with Services. You'll write Ingress resources, read `kubectl describe ingress`, and get tripped by the **`networking.k8s.io/v1` structure** (nested `service.port.number`, required `pathType`) that older course material predates. Knowing `kubectl create ingress` saves real time. Note the **"ingress" naming collision** with NetworkPolicy: this chapter is the actual `Ingress` *resource* (L7 routing), a different thing from the ingress *direction* in a NetworkPolicy.

---

## 1. The setup we're improving on

Build up the problem before naming the solution. An online store:
...
On a cloud (GCP): instead of NodePort + your own proxy, set `wear-service` to **`type: LoadBalancer`**, and GCP gives you a real load balancer with a public IP. DNS points at that.
...
That sprawl is exactly what Ingress exists to fix: one entry point, with host/path routing and TLS handled **inside** Kubernetes as config you version-control.
...
- **No cloud controller, no LB.** On bare metal, a VPS, or a local **kind** cluster, there's no CCM to fulfill the request, so the Service sits at `EXTERNAL-IP: <pending>` forever — unless you install something like **MetalLB** to play that role.
...
> **Real-world bridge:** a plain nginx reverse proxy — terminating TLS and routing hostnames/paths to containerized apps — *is* conceptually an ingress controller. The difference in Kubernetes is that you don't hand-edit `nginx.conf`; the controller **watches the cluster** and rewrites nginx config automatically as you add Ingress resources.

---

## 5. The Ingress Controller — the moving parts
...
**`POD_NAME` / `POD_NAMESPACE`:** the **Downward API** lets a container read facts about *its own* pod and inject them as environment variables. Here the controller needs to know **which namespace it's running in**, because its launch argument is:
...
**Version corrections (older course material is dated here):**
- Older material shows `apiVersion: extensions/v1beta1` for `kind: Deployment`. That's dead — **Deployments are `apps/v1`** (removed from `extensions` in 1.16).
...
- **Two different "nginx ingress" projects** exist and confuse everyone: the community **ingress-nginx** and **NGINX Inc.'s nginx-ingress**. Different annotations and features. For CKAD assume community ingress-nginx.
...
Older course material used the old API. On any current cluster (and the exam):

- **`apiVersion: networking.k8s.io/v1`.** `extensions/v1beta1` and `networking.k8s.io/v1beta1` are **removed** (gone in 1.22).
- **`pathType` is required** on every path: `Prefix`, `Exact`, or `ImplementationSpecific`. Omit it in v1 and the object is rejected.
...
> **Lab gotcha (nginx):** with path-based routing, a request to `/wear` arrives at the backend pod still pathed as `/wear`, but the `wear` app serves its content at `/`. You fix that with the nginx annotation `nginx.ingress.kubernetes.io/rewrite-target: /` (often with a regex capture in the path). When a route "connects but 404s," suspect rewrite-target.
